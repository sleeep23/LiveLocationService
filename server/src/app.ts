...
const supabase = createClient("https://mgzxxdgcudzhueouyorv.supabase.co", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1nenh4ZGdjdWR6aHVlb3V5b3J2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE2Njg2Njg0ODksImV4cCI6MTk4NDI0NDQ4OX0.kF02tvUzcZGTuWZj-QViobyi5pzcMMyhwAyGBU3j928", options);
...
const AddUser = async (userID: string, userName: string) => {
  const { error } = await supabase.from('user').insert({ user_id: userID, user_name: userName});
  if (error) console.log("Supabase DB insertion failed: ", error.message);
};
const RemoveUser = async(userID: string) => { // cascading problem -> solved by delete referenced tuples one by one
  const { error } = await supabase.from('chat_member').delete().match({user_id: userID});
...
const GetUserInfo = async(userID: string) => {
  const { data, error } = await supabase.from('user').select('user_name').eq('user_id', userID);
  return data?.pop();
}
const AddUserLocation = async (userID: string, location: LocationType) => {
  const { error } = await supabase.from('user_location').insert({ user_id: userID, lat: location.lat, lng: location.lng });
  if (error) console.log("Supabase DB insertion failed: ", error.message);
};
const updateUserLocation = async (userID: string, location: LocationType) => { // regular location update 
  const { error } = await supabase.from('user_location').update({lat: location.lat, lng:location.lng}).eq('user_id', 'userID');
};
...
const addRoomInfo = async (roomID: string, roomName: string) => {
  const { error } = await supabase.from('chat_room').insert({ room_id: roomID, room_name: roomName});
  if (error) console.log("Supabase DB insertion failed: ", error.message);
}

const addRoomMember = async (userIDs: any, roomID: string) => {
  userIDs.forEach(async (id: any) => {
    const {error} = await supabase.from('chat_member').insert({user_id: id, room_id: roomID});
    if(error) {
      console.log(error);
      exit(1);
    }
...
const deleteRoomData = async (userID: string, roomID: string) => {
  const { error } = await supabase.from('chat_member').delete().match({user_id: userID, room_id: roomID});
...
const GetChatMessage = async (roomId: any) => {
  const {data, error} = await supabase.from('chat_message').select('*').eq('room_id', roomId);
  if (!error) return data;
}

// ------------------- Supabase test code -------------------------
const getTable = async () => {
  const {data, error} = await supabase.from('user').select('user_name').in('user_id', ['asdfghkl', '12345', '23456']);
  let roomName = ``;
  data?.forEach((names) => {
...
const deleteTable = async () => {
  const { error } = await supabase.from('chat_member').delete().match({user_id: 'asdfghkl'});
...
io.on("connection", (socket) => {
  ...
  socket.on("update_location", (user) => { // receive regular update calls
    updateUserLocation(socket.id, user.location);
  })

  // receive socket id of a clicked client
  socket.on("createRoom", async (IDs: any) => { // IDs : array of participant's IDs;
    const {data, error} = await supabase.from('user').select('user_name').in('user_id', IDs);
  ...
  socket.on("get_rooms", async (user_id) => { 
    ...
    const {data, error} = await supabase.from('chat_member').select('room_id').eq('user_id', user_id);
    data?.forEach((item) => {
  ...
  socket.on("update_chat_messages", async (record) => { 
    const { error } = await supabase
                      .from('chat_message')
                      .insert({ user_id: record.socketID, room_id: record.roomID, message: record.message, tiem: record.time});
    if (!error) {
  ...
  socket.on("leaveRoom", async (userID: string, roomID: string) => {
    const { data, error } = await supabase.from('chat_room').select('room_name').eq('room_id', roomID);
    const roomName: any = data?.forEach((item) => item.room_name);
    const userName: any = GetUserInfo(userID)
    deleteRoomData(userID, roomID);
    socket.leave(roomName);
    socket.in(roomName).emit("leaveRoomMessage", `${userName.user_name} has left from the room`); 
